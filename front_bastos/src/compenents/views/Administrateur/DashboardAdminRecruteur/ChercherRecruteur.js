...
const ChercherRecruteur = () => {
  const [searchData, setSearchData] = useState({
    ...
    searchEmail: "",
    searchRole: "Recruteur", // Par défaut, recherche les "Recruteurs"
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/admin/getusers/3', {
        method: 'GET',
        headers: {
      ...
      const filteredUsers = users.filter(user =>
        searchData.searchNom?.toLowerCase() && user.nom.toLowerCase().includes(searchData.searchNom?.toLowerCase()) &&
        searchData.searchNumero_telephone?.toLowerCase() && user.numero_telephone?.toLowerCase().includes(searchData.searchNumero_telephone?.toLowerCase()) ||
      ...
      <form className="row col-12 mx-auto" onSubmit={handleSubmit}>
        <div className="row mb-3 col-md-12 col-12 mx-auto border border-2 border-black rounded rounded-20">
          ...
          <div className="mb-3 col-md-12 col-12">
            <label htmlFor="searchNom" className="bg-primary p-2 text-black font-bold rounded m-1 mx-auto col-md-3 col-12">
              Nom
            </label>
            <input
              type="text"
              ...
              className="bg-default  m-1 mx-auto col-md-3 col-12" // Classe Bootstrap pour un champ de formulaire
            />
          </div>

          {/* Champ de recherche par Téléphone */}
          <div className="mb-3 col-md-12 col-12">
            <label htmlFor="searchNumero_telephone" className="bg-primary p-2 text-black font-bold rounded m-1 mx-auto col-md-3 col-12">
              Téléphone
            </label>
            <input
              type="text"
...
export default ChercherRecruteur;